...
- **No personal names** (Darren, Morgs, Morgan, or any real identity)
- **No TheCryptoDonkey references** in NIPs submitted to nostr-protocol/nips — use `forgesworn/*` for public repos only
- **No RenegAid-specific terms** (e.g. `renegade` as a role name) — use generic equivalents
- **No private repo links** — `TheCryptoDonkey/trott-sdk`, `TheCryptoDonkey/trotters`, etc. are private
- **forgesworn/* links are fine** — this is the public GitHub org
- Run `grep -ri 'darren\|morgs\|morgan\|renegaid\|thecryptodonkey' nips/` before pushing
